      ...
      >
        <tbody>
          <tr
            ...
            :key="client.id"
          >
            <td data-label="#Sl" style="text-align: center">{{ client.id }}</td>
            ...
            <td data-label="NID" style="text-align: center">
              {{ client.nid }}
            </td>
            <td data-label="Action" style="text-align: center">
              <router-link :to="`/dashboard/edit_client/${client.id}`">
...
export default {
  name: "InterportClients",
  title: "Inter-Port - Client",
  ...
  data() {
    ...
    return {
      ...
      url: "http://127.0.0.1:8000/api/interport/v1/clients/export",
    };
  },
  ...
  methods: {
    ...
    async uploadFile() {
      ...
      try {
        await axios.post(
          "http://127.0.0.1:8000/api/interport/v1/clients/import",
          formData
        );
        location.reload();
    ...
    deleteClient: async function (client) {
      ...
      try {
        await this.$store.dispatch("client/delete_client", id).then(() => {
          this.$swal.fire({
            toast: true,
            position: "top-end",
            icon: "success",
