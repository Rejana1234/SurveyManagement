...
export default {
  ...
  computed: {
    ...mapState({
      edit_style: (state) => state.style.style,
  ...
  methods: {
    ...
    editStyle: async function () {
      try {
        ...
        formData.append("style_name", this.edit_style.style_name);
        formData.append("_method", "PUT");

        await this.$store
          .dispatch("style/update_style", { id: id, data: formData })
          .then(() => {
            this.$swal.fire({
              toast: true,
              position: "top-end",
